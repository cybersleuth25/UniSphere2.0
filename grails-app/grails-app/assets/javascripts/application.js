/**
 * UniSphere Application JavaScript
 * 
 * Migrated from script.js (PHP version).
 * 
 * Key changes from PHP version:
 *   - API endpoints updated: 'api.php' → '/api/posts', etc.
 *   - Auth endpoints: 'login.php' → '/auth/doLogin', 'signup.php' → '/auth/doSignup'
 *   - Profile endpoints: 'update-profile.php' → '/profile/update', 'upload-avatar.php' → '/profile/uploadAvatar'
 *   - Notification endpoint: 'notifications.php' → '/api/notifications'
 *   - Footer loading: 'footer.php' → rendered server-side via GSP template
 *   - Profile link: 'profile.php' → '/profile'
 *   - Login redirect: 'login.html' → '/login'
...
function fetchPostsAndRender(tab) {
  ...
  // CHANGED: api.php → /api/posts
  const url = `/api/posts?postType=${tab}${query ? '&search=' + encodeURIComponent(query) : ''}`;
  ...
  setTimeout(() => {
    fetch(url)
      ...
      .then(allPosts => {
        ...
        const cardsHTML = allPosts.map((x, index) => {
            ...
            const staggerDelay = (index % 4) + 1; // Generates stagger-1 to stagger-4

            return `<article class="card ${x.postType} stagger-${staggerDelay}" data-post-id="${x.id}" data-post-raw='${JSON.stringify(x)}'>
                ${imageHtml}
                <div class="card-content">
                    ...
                    <div class="post-meta meta" style="margin-top:auto">
                        <button class="like-btn btn secondary" data-post-id="${x.id}" style="padding: 6px 12px; font-size:12px;"><i class="fas fa-thumbs-up"></i> <span class="like-count">${x.likes || 0}</span></button>
                    </div>
...
function handleLikeClick(e) {
    ...
    fetch('/api/posts/like', { method: 'POST', body: formData })
        .then(res => res.json())
        .then(data => {
            if(data.success) {
...
function handleDeleteClick(e) {
    ...
    fetch(`/api/posts/${postId}`, { method: 'DELETE' }).then(res => res.json()).then(data => {
...
if (postModal) {
    ...
    postForm.addEventListener('submit', e => {
        ...
        if (!isUpdate) {
            ...
            fetch('/api/posts', { method: 'POST', body: formData }).then(res => res.json()).then(data => {
            ...
            fetch('/api/posts', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(postData) }).then(res => res.json()).then(data => {
...
function toggleTheme() {
  body.classList.toggle('dark-theme'); const isDark = body.classList.contains('dark-theme');
  localStorage.setItem('unisphere_theme_neo', isDark ? 'dark' : 'light');
}
if (themeToggleCheckbox) themeToggleCheckbox.addEventListener('change', toggleTheme);
const savedTheme = localStorage.getItem('unisphere_theme_neo');
// Default to light mode for neo-brutalism contrast, apply dark if explicitly set
if (savedTheme === 'dark') { body.classList.add('dark-theme'); if (themeToggleCheckbox) themeToggleCheckbox.checked = true; }
else { body.classList.remove('dark-theme'); if (themeToggleCheckbox) themeToggleCheckbox.checked = false; }
...
function checkLoginStatus() {
  ...
  if (authButtonsContainer) {
    if (isLoggedIn) {
        ...
        if (welcomeMessage) welcomeMessage.textContent = `HELLO, ${currentUser.username.toUpperCase()}!`;
        ...
        authButtonsContainer.innerHTML = `<button id="loginBtn" class="btn secondary">Login</button><a href="/signup" class="btn">Sign Up</a>`;
        const loginBtn = document.getElementById('loginBtn'); if (loginBtn) loginBtn.addEventListener('click', () => window.location.href = '/login');
...
if (loginForm) {
  loginForm.addEventListener('submit', (e) => {
    ...
    fetch('/auth/doLogin', { method: 'POST', body: formData }).then(response => response.json()).then(data => {
        if (data.success) { 
...
if (signupForm) {
  signupForm.addEventListener('submit', (e) => {
    ...
    fetch('/auth/doSignup', { method: 'POST', body: formData }).then(response => response.json()).then(data => {
        if (data.success) { localStorage.setItem(AUTH_KEY, JSON.stringify(data.user)); showToast(data.message); setTimeout(() => { window.location.href = '/'; }, 1000); }
        else { alert(data.message); }
    });
  });
}
...
function loadParticles() {
    // Disabled in favor of neo-brutalist static background texture
}

// REMOVED: loadFooter() - footer is now rendered server-side via GSP template
...
function renderGlobalSearchResults(data) {
    ...
    if (data.users.length > 0) {
        ...
        data.users.forEach(user => {
            ...
            // CHANGED: profile.php?username= → /profile/
            resultsHTML += `<article class="card"><div class="thumb image" style="background-image: url('${avatarUrl}')"></div><div class="card-content"><h3><a href="/profile/${user.username}">${user.username}</a></h3><p class="card-description">${user.bio || ''}</p></div></article>`;
        });
        resultsHTML += '</div>';
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // REMOVED: loadFooter() call - footer is rendered server-side
    checkLoginStatus();
    loadParticles();
    const currentPage = window.location.pathname;

    // CHANGED: '' or 'index.html' → '/'
    if (currentPage === '/' || currentPage === '') {
        ...
        if (searchInput) { 
            const debouncedSearch = debounce(() => {
                ...
                if (query.length > 2) {
                    // CHANGED: api.php?globalSearch= → /api/posts/search?globalSearch=
                    fetch(`/api/posts/search?globalSearch=${encodeURIComponent(query)}`)
                        .then(response => response.json())
    ...
    // CHANGED: 'profile.php' → '/profile'
    if (currentPage.startsWith('/profile') && typeof serverData !== 'undefined') {
        ...
        if(serverData.isOwnProfile) {
            ...
            fileInput.addEventListener('change', () => {
                ...
                // CHANGED: upload-avatar.php → /profile/uploadAvatar
                fetch('/profile/uploadAvatar', { method: 'POST', body: formData }).then(res => res.json()).then(data => {
                    if (data.success) {
            ...
            editProfileForm.addEventListener('submit', (e) => {
                ...
                // CHANGED: update-profile.php → /profile/update
...
if (forgotPasswordForm) {
  forgotPasswordForm.addEventListener('submit', (e) => {
    ...
    // CHANGED: forgot-password.php → /auth/doForgotPassword
    fetch('/auth/doForgotPassword', { method: 'POST', body: formData })
      .then(response => response.json())
      .then(data => {
        alert(data.message);
      })
